...
def calculates_results_stats(results_dic):
    ...
    for i in range(len(key_list)):
        ...
        if results_dic[key_list[i]][3] == 1:
            n_dogs_img += 1
        # number of matches between pet & classifier labels
        if results_dic[key_list[i]][2] == 1: #????
            n_match += 1
        # number of correctly classified dog images
        if results_dic[key_list[i]][3] == 1 and results_dic[key_list[i]][4] == 1:
            n_correct_dogs += 1
        # number of correctly classified dog breeds
        if results_dic[key_list[i]][2] == 1 and results_dic[key_list[i]][3] == 1:
            n_correct_breed += 1
    ...
    pct_correct_dogs = 0
    if n_images > 0:
        pct_correct_dogs = n_correct_dogs / n_images * 100
